        ...
        <section className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <table className="w-full text-sm">
                    ...
                    <tbody>
                        {loading ? (
                            <tr><td colSpan="3" className="p-10 text-center italic text-gray-400">Cargando...</td></tr>
                        ) : skillsList.length > 0 ? (
                            skillsList.map((skill) => (
                                <tr key={skill._id || Math.random()} className="border-t border-gray-100">
                                    <td className="px-5 py-4 font-semibold text-[#0F172A]">{skill.name}</td>
                                    <td className="px-5 py-4">{skill.categoryId?.name || "Global"}</td>
                            ...
                            <tr><td colSpan="3" className="p-10 text-center text-gray-400">No hay habilidades registradas.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
            {isModalOpen && <SkillModal onClose={() => setIsModalOpen(false)} />}
        </section>
    );
};